...
async function patchContext() {
  ...
  const next = `let streamingMessageId: string | null = null;
        const streamEventHandler = (event: any) => {
          if (event?.type === 'delta' && event.text) {
            ...
            setStatus(event.state === 'researching' ? 'EXECUTING' : 'THINKING');
            if (event.task?.taskType) {
              setPipeline((prev) => ({ ...prev, critic: { ...prev.critic, intent: event.task.taskType } }));
            }
  ...
  const oldAppend = "setMessages((prev) => streamingMessageId ? prev.map((m) => m.id === streamingMessageId ? ultronMsg : m) : [...prev, ultronMsg]);";
  const newAppend = "setMessages((prev) => streamingMessageId ? prev.map((m) => m.id === streamingMessageId ? ultronMsg : m) : [...prev, ultronMsg]);";
  if (content.includes(oldAppend)) content = content.replace(oldAppend, newAppend);
  await fs.writeFile(file, content, 'utf8');
}
...
async function main() {
  await patchApi();
  await patchContext();
  console.log('[Interface] Agent streaming/voice completion contract applied.');
}
