...
function buildConfig(models) {
  ...
  for (const model of models) {
    ...
    if (!id) continue;
    const entry = { name: String(model?.name || id) };
  ...
  return {
    ...
    provider: {
      omniroute: {
        ...
        options: { baseURL, apiKey: '{env:OMNIROUTE_API_KEY}' },
        models: modelMap,
      },
    },
  };
...
async function main() {
  ...
  process.stdout.write(JSON.stringify({ configPath, modelCount: models.length, provider: 'omniroute' }, null, 2));
}
