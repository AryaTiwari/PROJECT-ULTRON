...
function fixture({failAppendOnce=false}={}){
  const db=memoryDb(),timeline=[],appended=[];let appendFailures=failAppendOnce?1:0,readbacks=0,apolloCalls=0;
  const workspace={
    ...
    async googleSheetRead(){timeline.push("read");return{values:[["COMPANY NAME","COMPANY LINK","POC EMAIL"],["Existing Co","https://linkedin.com/company/existing",""]]};},
...
test("18 Google interruption resumes without rerunning Apollo",async()=>{const{f,started,mission}=await runFixture({failAppendOnce:true});assert.equal(mission.status,"blocked");const calls=f.apolloCalls;await f.runner.resume(started.runId);const completed=await waitFor(()=>f.db.getMission(started.runId)?.status==="completed"&&f.db.getMission(started.runId));assert.equal(completed.status,"completed");assert.equal(f.apolloCalls,calls);});
test("19 mission implementation never deletes company rows",()=>{const source=fs.readFileSync(new URL("../src/apollo-company-mission.mjs",import.meta.url),"utf8");assert.doesNotMatch(source,/deleteRows|batchClear|spreadsheets\.values\.clear|clear\(/);});
test("20 ordinary conversation remains generic Hermes conversation",()=>{const result=compileCommand("How should I plan my day?");assert.equal(result.owned,false);assert.equal(result.domain,"general-conversation");});
