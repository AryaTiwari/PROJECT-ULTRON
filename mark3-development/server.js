...
function serve(req, res) {
  ...
  res.writeHead(200, {
    ...
    'Cache-Control': /\.(js|css)$/i.test(target) ? 'no-cache' : 'no-store',
  });
...
function serveAudio(req, res) {
  ...
  if (!file.startsWith(`${audioRoot}${path.sep}`) || !fs.existsSync(file) || !fs.statSync(file).isFile()) {
    return send(res, 404, { ok: false, error: 'Audio file not found.' });
  }
  ...
  try {
    ...
    if (req.method === 'GET' && req.url === '/api/state') {
      return send(res, 200, {
        ...
        commitments: workspace.listCommitments({ status: 'open' }),
        projects: workspace.listProjects(),
        decisions: workspace.listDecisions(),
      });
    }
    ...
    if (req.method === 'GET' && req.url === '/api/diagnostics/omniroute') {
      ...
      try { catalog = await models.catalog(); }
      catch (error) { catalogError = error instanceof Error ? error.message : String(error); }
      const router = await integrations.health();
      return send(res, router.ok ? 200 : 503, {
        ok: Boolean(router.ok),
        endpoint: config.omnirouteBase,
        router,
        catalog,
        catalogError,
        credential: {
          envConfigured: Boolean(config.omnirouteEndpointKey),
          resolved: Boolean(await integrations.resolveOmniRouteApiKey()),
        },
      });
    }
    ...
    if (req.method === 'POST' && req.url === '/api/voice/test') {
      ...
      const audio = await integrations.speak(String(data.text || 'ULTRON Mark 3 voice online.'));
    ...
    if (req.method === 'POST' && req.url === '/api/chat') {
      const data = await body(req);
      const result = await assistant.handle(data.message, { model: data.model, history: data.history, taskType: data.taskType });
      return send(res, 200, result);
    }
    if (req.method === 'POST' && req.url === '/api/memory') {
      const data = await body(req);
      return send(res, 200, { ok: true, result: memory.remember(data) });
    }
    if (req.method === 'POST' && req.url === '/api/commitments') {
      const data = await body(req);
      return send(res, 200, { ok: true, commitment: workspace.createCommitment(data) });
    }
    if (req.method === 'POST' && req.url === '/api/decisions') {
      const data = await body(req);
      return send(res, 200, { ok: true, decision: workspace.recordDecision(data) });
    }
    if (req.method === 'POST' && req.url === '/api/projects') {
      const data = await body(req);
      return send(res, 200, { ok: true, project: workspace.upsertProject(data) });
    }
