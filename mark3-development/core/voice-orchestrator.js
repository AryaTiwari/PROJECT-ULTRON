...
function clean(text) {
  return String(text || '').replace(/```[\s\S]*?```/g, ' ').replace(/https?:\/\/\S+/gi, ' ').replace(/[#*_`>\[\]{}|~]/g, ' ').replace(/\s+/g, ' ').trim();
}
...
function splitSpeech(text, maxChars = 260) {
  ...
  for (const sentence of sentences) {
    ...
    for (const word of words) {
      ...
      if (next.length > maxChars && current) { chunks.push(current); current = word; } else current = next;
    }
...
async function speakChunk(text, index, total) {
  ...
  emit('voice_ready', { index, total, filename, path: audio.path });
  return audio;
}
...
function status() { return { speaking, queued: chunksPending(), provider: 'fish-audio-s2.1-pro-free' }; }
function chunksPending() { return speaking ? 1 : 0; }
