...
const MEANINGFUL = new Set([
  'task_started','task_completed','response_ready','tool_completed',
  'coding_completed','coding_failed',
  'reel_factory_started','reel_factory_completed','reel_factory_blocked','reel_factory_artifact_attached','reel_factory_artifact_failed',
  'reel_feedback_recorded','reel_performance_synced',
  ...
  'research_completed','research_failed','system_coach_suggestion',
...
function sourceFor(type = '', taskType = '') {
  const combined = `${type} ${taskType}`;
  if (/reel|creator-content/i.test(combined)) return 'reel';
  if (/forge|coding|developer/i.test(combined)) return 'forge';
  if (/adaptive/i.test(combined)) return 'adaptive';
  if (/instagram|buffer|social-publish/i.test(combined)) return 'instagram';
  if (/research/i.test(combined)) return 'research';
  if (/system_coach|diagnostic|system-audit/i.test(combined)) return 'diagnostics';
  if (/media|file|artifact/i.test(combined)) return 'artifacts';
  if (/task|response|tool/i.test(combined)) return 'assistant';
  return 'system';
}
...
function summarize(event = {}) {
  ...
  const candidates = [
    ...
    event.blocker, event.result?.title, event.result?.summary, event.response, event.error,
  ].filter(Boolean);
  ...
  if (type === 'reel_factory_completed') summary = clean(event.brief || event.output || 'Reel rendered and verified.', 190);
  if (type === 'reel_factory_blocked') summary = clean(`Reel blocked: ${event.blocker || 'production blocker'}`, 190);
...
function normalize(event = {}) {
  const taskType = event.taskType || event.requestedTaskType || null;
  return {
    id: event.id || `activity-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
    at: event.at || new Date().toISOString(),
    type: String(event.type || 'event'),
    source: sourceFor(event.type, taskType),
    summary: summarize(event),
    taskType,
    status: event.ok === false || /failed|blocked/i.test(String(event.type || '')) ? 'attention' : /completed|approved|verified|synced|attached/i.test(String(event.type || '')) ? 'done' : 'info',
