...
function isRetryableMissionError(error) {
  ...
  return /TIMEOUT|TIMED_OUT|TRANSIENT|ECONNRESET|ECONNREFUSED|EPIPE|NETWORK|FETCH|MCP_START_FAILED|MCP_CONNECTION|GOOGLE_SHEETS_API_ERROR|HTTP_5\d\d|TEMPORAR|ANOTHER LINKEDIN MCP CLIENT|BROWSER.*(?:BUSY|USING)|CURRENTLY USING THE BROWSER/i.test(code + ' ' + text);
}
function list() {
