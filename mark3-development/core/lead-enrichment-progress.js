...
async function recoverContextualPhones(sheetUrl, options = {}) {
  ...
  let layout;
  try {
    layout = await adapter.inspect(sheetUrl);
  } catch {
  ...
  let data;
  try {
    data = await adapter.readSheet(sheetUrl, layout);
  } catch {
    return { recovered: 0, skipped: true };
  }
...
function summarizeJob(job, now = Date.now()) {
  if (!job) return null;
  const rows = Object.values(job.rows || {});
  const pendingRows = rows.filter((row) => row?.phonePending);
  const resolvedRows = rows.filter((row) => row?.phoneResolved);
  const failedRows = rows.filter((row) => row?.error && !row?.phonePending);
  ...
  const initialPending = Number(job.stats?.pendingPhones || rows.filter((row) => row?.requestedAt).length || 0);

  return {
    jobId: job.id || null,
    provider: job.provider || 'google',
    spreadsheetTitle: job.spreadsheetTitle || null,
    sheetName: job.sheetName || null,
    status: job.status || null,
    scannedRows: Number(job.stats?.scannedRows || 0),
    liveCalls: Number(job.stats?.enrichedProfiles || 0),
    cacheHits: Number(job.stats?.cachedProfiles || 0),
    localEmails: Number(job.stats?.sheetEmailsRecovered || 0),
    localPhones: Number(job.stats?.sheetPhonesRecovered || 0) + Number(job.stats?.contextualPhonesRecovered || 0),
    initialPending,
...
function latestJobSummary(state = leadEnrichment.loadState()) {
  const jobs = Array.isArray(state?.jobs) ? state.jobs : [];
  const latest = [...jobs].reverse().find((job) => job && (job.stats || Object.keys(job.rows || {}).length));
...
function buildStatusText(runtime, summary, sync = {}) {
  ...
  if (summary) {
    ...
    const returned = summary.resolved;
    text += ` Latest run ${location}: ${summary.scannedRows} LinkedIn rows checked. Phone verification started for ${summary.initialPending}; ${returned} callback result${returned === 1 ? '' : 's'} processed, ${summary.pending} still pending.`;
    if (returned) text += ` Returned phones: ${summary.resolvedWithPhone} found, ${summary.resolvedNoPhone} confirmed no phone.`;
    text += ` Local-first recovered ${summary.localEmails} emails and ${summary.localPhones} phones. Apollo live calls: ${summary.liveCalls}; cache hits: ${summary.cacheHits}.`;
  ...
  if (sync?.error) text += ` Webhook sync could not refresh just now: ${sync.error}. No spreadsheet cells were overwritten because of that sync failure.`;
  else text += ` This status check made 0 new Apollo enrichment calls; it only synced already-requested webhook results.`;
  text += ' Explicit approval remains mandatory before any new Apollo enrichment call.';
  return text;
}

function responseShape(text, summary, sync) {
  return {
    ok: true,
    response: text,
    text,
...
function install() {
  ...
  assistant.handle = async (message, options = {}) => {
    ...
    const result = responseShape(status.text, status.summary, status.sync);
