...
function summary(mission) {
  ...
  return {
    ...
    phase,
    completionState: m.completionState || phase,
    running: active.has(m.missionId),
    targetCount: Number(m.targetCount || 0),
    companyCandidatesFound: Number(m.companyCandidatesFound || 0),
    candidatePoolTarget: Number(m.candidatePoolTarget || 0),
    companiesQualified: Number(m.companiesQualified || 0),
    companiesSelected: Number(m.companiesSelected || 0),
    companiesWritten: Number(m.companiesWritten ?? m.rowsWritten ?? 0),
    companiesAlreadyExisting: Number(m.companiesAlreadyExisting ?? m.skippedDuplicates ?? 0),
    searchVariantsTried: Number(m.searchVariantsTried || 0),
    searchVariants: m.searchVariants || m.searchDiagnostics || [],
    organizationSearchCalls: Number(m.organizationSearchCalls || 0),
    peopleSearchCalls: Number(m.peopleSearchCalls || 0),
    contactRevealCalls: Number(m.contactRevealCalls || 0),
    cacheHits: Number(m.cacheHits || 0),
    apolloCalls: Number(m.apolloCalls || 0),
    paidCalls: Number(m.paidCalls || 0),
    POC1Selected: Number(m.POC1Selected || 0),
    POC2Selected: Number(m.POC2Selected || 0),
    indianPhoneAttempts: Number(m.indianPhoneAttempts || 0),
    foreignFallbacks: Number(m.foreignFallbacks || 0),
    phonesFilled: Number(m.phonesFilled || 0),
    emailsFilled: Number(m.emailsFilled || 0),
    unresolvedContactSlots: Number(m.unresolvedContactSlots || 0),
    pendingRevealIds: m.pendingRevealIds || [],
    ...
    spreadsheetId: m.spreadsheetId || m.compiledFilters?.sheet?.spreadsheetId || null,
    sheetId: m.sheetId ?? m.compiledFilters?.sheet?.sheetId ?? null,
    sheetName: m.exactWorksheetTitle || m.sheetName || m.compiledFilters?.sheet?.exactTitle || m.compiledFilters?.sheet?.sheetName || null,
    sheetUrl: m.worksheetUrl || m.sheetUrl || m.compiledFilters?.sheet?.url || null,
    approvalId: m.approvalId || null,
    runtimeBuild: m.runtimeBuild || null,
