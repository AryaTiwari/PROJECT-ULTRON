...
const TOOLS = [
  {
    id: 'tavily', name: 'Tavily Search', category: 'research', priority: 1,
  ...
  {
    id: 'firecrawl', name: 'Firecrawl', category: 'web-extraction', priority: 2,
    env: ['FIRECRAWL_API_KEY'], auth: 'api-key',
    free: '1,000 credits/month; no card required',
    purpose: 'Reliable page extraction/crawling and browser-like interaction for sites direct HTTP cannot read.',
    zeroCostSafe: true, autoUse: 'fallback-only',
  },
  {
    id: 'cloudflare-r2', name: 'Cloudflare R2', category: 'storage', priority: 1,
  ...
  {
    id: 'cloudflare-workers-ai', name: 'Cloudflare Workers AI', category: 'inference', priority: 2,
  ...
  {
    id: 'youtube-data', name: 'YouTube Data API', category: 'creator-intelligence', priority: 1,
  ...
  {
    id: 'resend', name: 'Resend', category: 'email', priority: 2,
  ...
  {
    id: 'telegram-bot', name: 'Telegram Bot API', category: 'remote-interface', priority: 2,
  ...
  {
    id: 'alpha-vantage', name: 'Alpha Vantage', category: 'market-research', priority: 3,
  ...
  {
    id: 'posthog', name: 'PostHog', category: 'product-analytics', priority: 2,
  ...
  {
    id: 'brave-search', name: 'Brave Search API', category: 'research', priority: 4,
  ...
  {
    id: 'apify', name: 'Apify', category: 'automation-research', priority: 4,
  ...
  {
    id: 'gmail-oauth', name: 'Gmail API', category: 'personal-operator', priority: 1,
  ...
  {
    id: 'calendar-oauth', name: 'Google Calendar API', category: 'personal-operator', priority: 1,
    env: ['GOOGLE_CLIENT_ID', 'GOOGLE_CLIENT_SECRET'], auth: 'oauth',
    free: 'Standard API usage is no-additional-cost below current daily thresholds',
  ...
  {
    id: 'gdelt', name: 'GDELT', category: 'news-intelligence', priority: 3,
...
function state(tool) {
  const ready = !tool.env.length || tool.env.every((name) => configured(name));
  const missing = tool.env.filter((name) => !configured(name));
  return { ...tool, configured: ready, missing };
}
function status() {
  ...
  return {
    ...
    availableToAdd: rows.filter((row) => !row.configured).sort((a, b) => a.priority - b.priority),
    total: rows.length,
  };
}
function nextRecommendations(limit = 6) {
  return status().availableToAdd.filter((row) => row.priority <= 3).slice(0, Math.max(1, limit));
}
