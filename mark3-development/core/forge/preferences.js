const founderRecipes = require('./founder-recipes');
...
function shouldDelegate(message = '') {
  ...
  const action = /\b(?:build|create|develop|implement|automate|integrate|connect|design|make|repair|upgrade)\b/i.test(text);
  if (founderRecipes.match(text) && action) return true;
...
function commonAcceptance(objective = '') {
  return [
    'Preserve existing working behavior',
    'No paid inference or paid fallback',
    'Run executable validation',
    'Report exact remaining blockers honestly',
    ...founderRecipes.acceptance(objective),
  ];
}

function fallbackJobs(objective, profile = classify(objective)) {
  const common = commonAcceptance(objective);
  const recipe = founderRecipes.match(objective);
  const recipeContract = recipe ? ` Follow founder recipe ${recipe.id}: ${recipe.requiredLayers.join(' → ')}.` : '';
  if (profile.id === 'repair') return [
    ...
    { id: 'patch', title: 'Minimal repair', objective: `Patch only the proven root cause while preserving working behavior.${recipeContract}`, kind: 'integration', worker: 'coding', dependsOn: ['reproduce'], acceptance: common },
    { id: 'regression', title: 'Regression proof', objective: 'Add or strengthen regression coverage and run the relevant checks.', kind: 'qa', worker: 'review', dependsOn: ['patch'], acceptance: ['Original failure no longer reproduces', 'Relevant existing checks still pass', ...founderRecipes.acceptance(objective)] },
  ];
  if (profile.id === 'integration') return [
    { id: 'contract', title: 'Integration contract', objective: `Define the exact API/OAuth/webhook contract, permissions, credential boundary, read/write operations and failure modes for: ${objective}.${recipeContract}`, kind: 'architect', worker: 'reasoning', dependsOn: [], acceptance: ['No secrets in code/logs', 'Read/write permission boundary is explicit', ...founderRecipes.acceptance(objective)] },
    ...
    { id: 'integration-review', title: 'Integration final review', objective: 'Verify permissions, retry behavior, secret hygiene, dry-run/live separation, founder-recipe outcome and restart safety.', kind: 'critic', worker: 'review', dependsOn: ['write-path'], acceptance: common },
  ];
  if (profile.id === 'automation') return [
    { id: 'workflow-contract', title: 'Automation contract', objective: `Define trigger, state, idempotency key, dedupe rules, retry policy, checkpoints, approval gates and success evidence for: ${objective}.${recipeContract}`, kind: 'automation', worker: 'reasoning', dependsOn: [], acceptance: ['Trigger and terminal states are explicit', 'Duplicate execution is prevented', ...founderRecipes.acceptance(objective)] },
    { id: 'state-engine', title: 'State and checkpoint layer', objective: 'Implement durable state/checkpoints by reusing existing Ultron workspace/memory where suitable so the automation resumes safely after restart or provider failure.', kind: 'backend', worker: 'coding', dependsOn: ['workflow-contract'], acceptance: common },
    ...
    { id: 'automation-qa', title: 'Restart + idempotency QA', objective: 'Simulate retry/restart/duplicate-trigger scenarios and verify no duplicate external action occurs.', kind: 'qa', worker: 'review', dependsOn: ['failure-recovery'], acceptance: ['Restart safety verified', 'Idempotency verified', ...founderRecipes.acceptance(objective)] },
  ];
  if (profile.id === 'creator_ops') return [
    { id: 'operator-contract', title: 'Creator operator contract', objective: `Define creator input, account/data source, business objective, measurable output, approval boundary and state transitions for: ${objective}.${recipeContract}`, kind: 'product', worker: 'reasoning', dependsOn: [], acceptance: ['Creator outcome is measurable', 'External actions are approval-gated', ...founderRecipes.acceptance(objective)] },
    ...
    { id: 'creator-review', title: 'Creator operator final review', objective: 'Verify founder UX, measurable outcome, approval safety, state continuity, recipe outcome and cross-feature data reuse.', kind: 'critic', worker: 'review', dependsOn: ['learning-hook'], acceptance: common },
  ];
  if (profile.id === 'media_pipeline') return [
    { id: 'creative-contract', title: 'Creative intelligence contract', objective: `Define audience goal, account aesthetic, current format evidence, script density, narrator, text/graphics grammar and final quality bar for: ${objective}.${recipeContract}`, kind: 'product', worker: 'reasoning', dependsOn: [], acceptance: ['Creative decisions are explainable', 'No trend is copied blindly', ...founderRecipes.acceptance(objective)] },
  ...
  if (profile.id === 'full_product') return [
    { id: 'vertical-slice', title: 'Smallest complete vertical slice', objective: `Define the smallest end-to-end slice that proves the value of: ${objective}.${recipeContract}`, kind: 'product', worker: 'reasoning', dependsOn: [], acceptance: ['Scope is minimal but complete', ...founderRecipes.acceptance(objective)] },
  ...
  return [
    { id: 'feature-contract', title: 'Feature contract', objective: `Define exact behavior, integration points and regression boundaries for: ${objective}.${recipeContract}`, kind: 'product', worker: 'reasoning', dependsOn: [], acceptance: ['Existing behavior to preserve is explicit', ...founderRecipes.acceptance(objective)] },
...
function compilerGuidance(objective = '') {
  ...
  return [
    ...
    founderRecipes.guidance(objective),
...
module.exports = { CURRENT_FREE_MODELS, PROFILES, classify, shouldDelegate, fallbackJobs, compilerGuidance, applyCurrentModelPool, founderRecipes };
