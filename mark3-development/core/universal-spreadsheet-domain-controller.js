...
function approvalSummary(inspection) {
  ...
  return [
    ...
    'After the deterministic/manual pass, bounded AI rescue receives only the exact row numbers explicitly left unresolved for POC-2. Row-evidence employer resolution runs before AI context, so Gemini context is used only when deterministic company/domain evidence is genuinely absent. AI does not reprocess verified partial POCs or phone-pending rows. Selection prefers Groq, then Gemini, then NVIDIA on failure. Reviewer is disabled by default. Maximum 3 direct env-backed AI attempts for the entire run, not per row. OmniRoute is not used by the direct batch path.',
