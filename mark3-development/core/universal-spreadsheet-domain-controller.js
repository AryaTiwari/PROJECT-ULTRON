...
function approvalSummary(inspection) {
  ...
  return [
    ...
    'Priority contract: execution is sheet-wide and phase-ordered. Phase 1 completes POC-1 contact fields across every eligible row from the exact anchor identity. Phase 2 re-reads the live sheet, completes existing POC-2 identities first, and discovers a new POC-2 only when the POC-2 name cell is actually blank. Phase 3 re-reads again, completes existing POC-3 identities first, and may run its own deterministic discovery only when the POC-3 identity is blank. Existing identities are contact-completion jobs: exact person evidence may fill missing phone/email even when row employer context is stale; employer verification remains mandatory whenever ULTRON selects a new person.',
