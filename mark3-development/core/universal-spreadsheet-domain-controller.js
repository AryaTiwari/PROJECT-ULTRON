...
function approvalSummary(inspection, policy = {}) {
  ...
  return [
    ...
    policy.requireIndianPhone
      ? `Indian-number contact gate${policy.indianPhonePolicySource === 'automatic-poc1-poc2-default' ? ' (automatic POC-1/POC-2 default)' : ''}: only POC-1 and POC-2 are eligible; India-located hiring decision-makers rank first. The proven enrichment pipeline remains active, including deterministic POC-2 rechecks and bounded AI rescue over verified candidates. Phone reveals stay limited to the final top decision-maker shortlist: two primary candidates plus at most one fallback when POC-2 remains unresolved. A POC qualifies for the Indian-number preference only when POC-1 or POC-2 has verified +91 mobile evidence. Foreign-only or no-phone POC candidates remain unresolved after bounded checks; the existing company row is always preserved and Apollo enrichment has no authority to clear or delete it. Pending exact Apollo callbacks remain staged.`
