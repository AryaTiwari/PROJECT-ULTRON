...
function approvalSummary(inspection) {
  ...
  return [
    ...
    'For a FINAL verified POC whose phone is still blank, ULTRON may use Apollo phone waterfall with poll_only/request-id polling. It never waterfalls discovery candidates, never enables personal-email reveal, and uses one phone strategy per verified POC rather than buying both native reveal and waterfall in the same run. Pending waterfall requests are persisted by exact sheet row/cell and polled in the background. Email waterfall remains bounded to final verified POCs.',
    'Bounded AI receives only exact POC-2 rows still unresolved after manual verification, and only supplied candidates. Row-evidence employer resolution runs before AI context. Selection prefers Groq, then Gemini, then NVIDIA on failure. Reviewer is disabled by default. Maximum 3 direct env-backed AI attempts for the entire run, not per row. OmniRoute is not used by the direct batch path.',
