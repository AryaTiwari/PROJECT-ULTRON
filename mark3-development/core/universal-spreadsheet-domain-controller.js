...
function approvalSummary(inspection) {
  ...
  return [
    ...
    'Priority contract: POC-1 is non-negotiable and exact-person completion runs first when contact data is missing; already-complete POC-1 rows are preserved without redundant hydration. Existing/partial POC-2 remains deterministic exact-person repair. For an EMPTY POC-2, ULTRON resolves the hiring organization, discovers candidates and runs deterministic manual priority/hydration first. Only rows that remain empty after manual verification are deferred to bounded AI rescue. POC-3 remains optional and never triggers discovery or AI by itself.',
    'After deterministic employer resolution, POC-2 discovery tries targeted Apollo, bounded broad Apollo, and one brand-keyword Apollo retry. If all Apollo discovery variants return zero, ULTRON may use authenticated read-only LinkedIn company/people discovery under the existing account-safety policy; LinkedIn supplies profile references and Apollo must still verify exact identity and employer before any write. Manual deterministic candidate selection and hydration always run before bounded AI. Bounded AI receives only exact POC-2 rows still unresolved after manual verification, and only candidates that already pass the same canonical employer matcher used by Apollo hydration. Row-evidence employer resolution runs before AI context. Selection prefers Groq, then Gemini, then NVIDIA on failure. Reviewer is disabled by default. Maximum 3 direct env-backed AI attempts for the entire run, not per row. OmniRoute is not used by the direct batch path.',
