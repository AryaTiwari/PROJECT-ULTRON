...
function approvalSummary(inspection) {
  ...
  return [
    ...
    'Priority contract: POC-1 is non-negotiable and exact-person completion runs first. POC-2 is the primary additional contact: ULTRON uses a fast priority-title Apollo search first (Founder/Director/Owner > recruiting/HR Head or Manager > Recruiter), then up to three deterministic verified hydration attempts before AI. POC-3 is optional and receives only one cheap manual attempt from the same fast pool; it does not consume AI rescue by default.',
    'After the deterministic/manual pass, bounded AI rescue receives only the exact row numbers explicitly left unresolved for POC-2. It does not reprocess verified partial POCs or phone-pending rows. Known employers skip the context model entirely. Selection prefers Groq, then Gemini, then NVIDIA on failure. Reviewer is disabled by default. Maximum 3 direct env-backed AI attempts for the entire run, not per row. OmniRoute is not used by the direct batch path.',
