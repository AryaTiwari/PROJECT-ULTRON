...
function approvalSummary(inspection) {
  ...
  return [
    ...
    'After the deterministic pass, a bounded AI batch rescue is available for unresolved POC slots. It uses one whole-run context pass, one whole-run candidate-selection pass, and only when needed one reviewer pass: maximum 3 direct AI attempts for the entire run, not per row. This workflow uses env-only direct providers: Gemini is preferred for context, Grok/xAI for candidate selection and NVIDIA for the optional reviewer; OmniRoute is not used.',
