...
function approvalSummary(inspection, policy = {}) {
  ...
  return [
    ...
    policy.requireIndianPhone
      ? `Indian-number preference${policy.indianPhonePolicySource === 'automatic-poc1-poc2-default' ? ' (automatic POC-1/POC-2 default)' : ''}: only POC-1 and POC-2 are eligible; India-located hiring decision-makers and verified +91 mobiles rank first. ULTRON checks only a bounded finalist shortlist of up to four distinct people per POC and never scans every employee. If no verified +91 number is available after those checks, a verified foreign phone is accepted as fallback; if no phone is available, the company row remains in place with the unresolved contact cells blank or pending. Existing company values are never cleared or deleted.`
