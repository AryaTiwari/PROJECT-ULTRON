...
function approvalSummary(inspection) {
  ...
  return [
    ...
    'Priority contract: POC-1 is non-negotiable and exact-person completion runs first when contact data is missing; already-complete POC-1 rows are preserved without redundant hydration. For POC-2, ULTRON first resolves the hiring organization from deterministic row evidence such as explicit company wording or business-email domains, repairs any existing POC exactly by LinkedIn/name+verified company, then runs fast priority-title Apollo discovery only when POC-2 is actually empty. POC-3 is optional and may only reuse a POC-2 discovery pool; it never triggers discovery by itself.',
    'After deterministic employer resolution, POC-2 discovery tries targeted Apollo, bounded broad Apollo, and one brand-keyword Apollo retry. If all Apollo discovery variants return zero, ULTRON may use one authenticated read-only LinkedIn people search under the existing account-safety policy; LinkedIn supplies only profile references and Apollo must still verify exact identity and employer before any candidate can be selected or written. Bounded AI rescue then receives only exact unresolved POC-2 rows. Row-evidence employer resolution runs before AI context, so Gemini context is used only when deterministic company/domain evidence is genuinely absent. AI does not reprocess verified partial POCs or phone-pending rows. Selection prefers Groq, then Gemini, then NVIDIA on failure. Reviewer is disabled by default. Maximum 3 direct env-backed AI attempts for the entire run, not per row. OmniRoute is not used by the direct batch path.',
    ...
    'Completion rule: partial progress is never enough. Any mandatory unresolved POC-2 row continues through the next safe strategy even when sibling rows were solved. After manual + direct AI + exact-row last resort, ULTRON re-reads the live sheet and may close only as COMPLETE or TERMINAL_EXHAUSTED with exact unresolved rows/reasons. POC-3 never blocks completion.',
