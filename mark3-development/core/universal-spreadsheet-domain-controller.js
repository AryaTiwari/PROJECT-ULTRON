...
function approvalSummary(inspection) {
  ...
  return [
    ...
    'Priority contract: POC-1 is non-negotiable and exact-person completion runs first. POC-2 is the primary additional contact: deterministic manual priority (Founder/Director/Owner > recruiting/HR Head or Manager > Recruiter) runs before AI. POC-3 is optional and receives only one cheap manual attempt; it does not consume AI rescue by default.',
    'After the deterministic/manual pass, bounded AI rescue is available only for unresolved POC-2. Known employers skip the context model entirely. Selection prefers Groq, then Gemini, then NVIDIA on failure. Reviewer is disabled by default. Maximum 3 direct env-backed AI attempts for the entire run, not per row. OmniRoute is not used by the direct batch path.',
