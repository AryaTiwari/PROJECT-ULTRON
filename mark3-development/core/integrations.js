...
async function githubReadFile(pathname, ref = config.githubBranch) {
  ...
  const url = `https://api.github.com/repos/${encodeURIComponent(config.githubOwner)}/${encodeURIComponent(config.githubRepo)}/contents/${pathname.split('/').map(encodeURIComponent).join('/')}?ref=${encodeURIComponent(ref)}`;
  const data = await jsonRequest(url, { headers: githubHeaders() }, 20000);
  if (data.type !== 'file' || !data.content) throw new Error(`GitHub path is not a readable file: ${pathname}`);
  return { path: pathname, ref, sha: data.sha, content: Buffer.from(data.content.replace(/\n/g, ''), 'base64').toString('utf8'), size: data.size || 0 };
}

async function githubList(pathname = '', ref = config.githubBranch) {
  if (!config.githubToken) throw new Error('GITHUB_TOKEN is not configured.');
  const suffix = pathname ? `/${pathname.split('/').map(encodeURIComponent).join('/')}` : '';
  ...
  return jsonRequest(url, { headers: githubHeaders() }, 20000);
}

async function models() {
  return jsonRequest(`${config.omnirouteBase}/models`, { headers: config.omnirouteApiKey ? { Authorization: `Bearer ${config.omnirouteApiKey}` } : {} }, 15000);
}

async function chat(messages, model = 'auto', tools = []) {
  const body = { model, messages, stream: false };
  if (tools.length) body.tools = tools;
  return jsonRequest(`${config.omnirouteBase}/chat/completions`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(config.omnirouteApiKey ? { Authorization: `Bearer ${config.omnirouteApiKey}` } : {}) }, body: JSON.stringify(body) }, 120000);
}

async function speak(text) {
  return jsonRequest(`${config.parentCore}/api/tts`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text, provider: 'fish-audio-s2.1-pro-free', format: 'mp3', volume: 2, temperature: 0.70, topP: 0.76, prosody: { speed: 1, volume: 2, normalize_loudness: true }, chunkLength: 240, conditionOnPreviousChunks: true }) }, 120000);
}

module.exports = { jsonRequest, githubReadFile, githubList, models, chat, speak };
