...
// Mark 3 deliberately uses Mark 2's provider selection. It does not pick a
// provider itself and therefore cannot accidentally force NVIDIA/OpenCode.
async function selectMark2Model(model = 'auto', taskType = 'general') {
  const requested = String(model || 'auto').trim();
  if (requested && !isRoutingAlias(requested) && !isBigPickle(requested) && !isOpenCodeModel(requested)) return requested;
  if (requested && (isBigPickle(requested) || isOpenCodeModel(requested))) throw new Error(`Disabled model in Mark 3: ${requested}`);
  return 'auto';
}
...
async function chat(messages, model = 'auto', tools = null, options = {}) {
  return parentRouter.chat({ messages, model: await selectMark2Model(model, options.taskType || 'general'), tools, taskType: options.taskType || 'general' });
}
async function streamChat(messages, model = 'auto', tools = null, options = {}) {
  return parentRouter.streamChat({ messages, model: await selectMark2Model(model, options.taskType || 'general'), tools, taskType: options.taskType || 'general', onDelta: options.onDelta, firstTokenTimeoutMs: options.firstTokenTimeoutMs });
}
async function chatExact(messages, model, tools) {
  ...
  return parentRouter.chat({ messages, model, tools });
}
async function health() { return parentRouter.health(); }
function providerHealthSnapshot() { return []; }
async function speak(text) {
  const response = await fetch(`${config.parentCore}/api/tts`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({text,provider:'fish-audio-s2.1-pro-free',format:'mp3',volume:2}) });
  const raw = await response.text();
  let data={}; try{data=raw?JSON.parse(raw):{};}catch{data={raw};}
  if(!response.ok){const error=new Error(`ULTRON TTS HTTP ${response.status}: ${String(raw).slice(0,1200)}`);error.status=response.status;throw error;}
  return data;
}
async function requestJson(url, options = {}, timeoutMs = 30000) {
  const controller = new AbortController(); const timer=setTimeout(()=>controller.abort(),timeoutMs);
  try { const response=await fetch(url,{...(options||{}),signal:controller.signal}); const text=await response.text(); let data={}; try{data=text?JSON.parse(text):{};}catch{data={raw:text};} if(!response.ok){const error=new Error(`HTTP ${response.status}: ${text.slice(0,2500)}`);error.status=response.status;error.body=data;throw error;} return data; } finally { clearTimeout(timer); }
}
...
async function githubList(pathname='',ref) {
  if (!config.githubToken) throw new Error('GITHUB_TOKEN is not configured.');
  const useRef=ref||config.githubBranch; const suffix=pathname?'/'+String(pathname).split('/').filter(Boolean).map(encodeURIComponent).join('/') : '';
