...
async function requestJson(url, options = {}, timeoutMs = 30000) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`${response.status}: ${data.message || data.error || text.slice(0, 500)}`);
    }
    return data;
  } finally {
    clearTimeout(timer);
...
async function githubReadFile(pathname, ref = config.githubBranch) {
  ...
  if (data.type !== 'file' || !data.content) {
    throw new Error(`GitHub path is not a readable file: ${pathname}`);
  }
...
async function models() {
  return requestJson(`${config.omnirouteBase}/models`, {
    headers: config.omnirouteApiKey
      ? { Authorization: `Bearer ${config.omnirouteApiKey}` }
      : {},
  }, 15000);
}
