...
function defaultState() {
  return { version: 1, templates: [], pendingPlan: null, missions: [] };
}
...
function saveState(state) {
  ...
  state.missions = (state.missions || []).slice(-20);
...
function ensureCoreHeaders(headers, wantsContacts = false) {
  ...
  return out.length ? out : DEFAULT_HEADERS.slice();
}
...
function parseCount(text) {
  ...
  const nearFind = value.match(/\b(?:find|research|discover|source|collect|get|bring|build|generate)\s+(?:me\s+)?(\d{1,4})\b/i);
...
function isWorkspaceRequest(text) {
  ...
  const discovery = /\b(?:find|research|discover|source|collect|get|bring|build|generate|look\s*up|scrape)\b/i.test(value);
  const destination = /\b(?:sheet|spreadsheet|lead list|lead database|lead sheet)\b/i.test(value);
  const direct = /\bbring\s+me\b[\s\S]{0,100}\bleads?\b/i.test(value);
...
async function createSpreadsheet(title, headers, rowTarget = 100) {
  ...
  const rowCount = Math.max(200, Math.min(2500, Number(rowTarget || 100) + 50));
...
function latestTemplate() {
  const state = loadState();
  return [...(state.templates || [])].sort((a, b) => String(b.lastUsedAt || b.learnedAt || '').localeCompare(String(a.lastUsedAt || a.learnedAt || '')))[0] || null;
}
...
function queryPlan(criteria, count) {
  ...
  const quoted = c.length <= 80 ? `"${c}"` : c;
  const modifiers = ['founder', 'co-founder', 'recruiter', 'talent acquisition', 'hiring manager', 'head', 'director', 'manager', 'consultant', 'creator', 'specialist', 'lead'];
  const queries = [
  ...
  const maxQueries = Math.min(28, Math.max(5, Math.ceil(Number(count || 25) / 6) + 4));
...
async function deepPublicContactPass(leads, criteria, mission, saveMission) {
  ...
  const maxLeads = Math.min(candidates.length, leads.length > 75 ? 10 : 20);
  let fetches = 0;
  let searches = 0;
  for (const lead of candidates.slice(0, maxLeads)) {
    if (!lead.name) continue;
    const company = lead.company ? `"${lead.company}"` : criteria;
    const q = `"${lead.name}" ${company} email phone contact`;
    try {
      const result = await web.searchWeb(q, { limit: 5 });
      searches++;
      for (const item of result.results || []) {
        ...
        if ((lead.email && lead.phone) || fetches >= 12) break;
        if (!publicScrapeCandidate(item?.url)) continue;
        try {
          const page = await web.fetchPage(item.url, { maxTextChars: 16000 });
          fetches++;
...
async function existingLinkedIns(mission) {
  try {
    ...
    for (let i = 1; i < values.length; i++) {
      const url = leadEnrichment ? require('./apollo-enrichment').normalizeLinkedIn(values[i]?.[idx]) : null;
      if (url) set.add(url);
    }
    return set;
  } catch {
...
async function continueMission(id) {
  ...
  try {
    ...
    const seen = new Set((mission.leads || []).map((lead) => lead.linkedin).filter(Boolean));
    ...
    const freshLeads = mission.leads.filter((lead) => !existing.has(lead.linkedin));
...
async function prepareRequest(plan) {
  ...
  return {
    type: 'clarification',
    text: `I can create the Google Sheet and run this as a lead mission. Your latest reusable layout${source} is: ${templatePreview(template)}. Use the previous format, use the default lead format, or send a different layout as “headers: Name, Company, LinkedIn, Phone, Email”.`,
    pending,
  };
}
...
async function resolvePending(text) {
  ...
  if (/\b(?:use|same as|keep|go with)\b[\s\S]{0,30}\b(?:previous|last|same|that)\b|\bprevious format\b|\bsame format\b|\buse it\b/i.test(value)) {
...
function statusText() {
  ...
  if (!mission) {
    return `Lead Workspace is ready. Google Sheet creation, public-web lead research, bounded public-page scraping and format memory are available. Latest remembered layout: ${templatePreview(template)}.`;
  }
  const shortfall = Number(mission.leads?.length || 0) < Number(mission.requested || 0)
    ? ` ${mission.leads?.length || 0}/${mission.requested} unique public LinkedIn leads discovered so far.`
    : ` ${mission.requested} requested leads reached.`;
  return `Lead mission ${mission.status}: ${mission.criteria}.${shortfall} ${mission.added || 0} rows written to ${mission.spreadsheetTitle || 'Google Sheet'}. Public contacts found: ${mission.publicEmails || 0} emails, ${mission.publicPhones || 0} phones. Search progress ${mission.queryIndex || 0}/${mission.queries?.length || 0}; deep public-page fetches ${mission.deepFetches || 0}. Sheet: ${mission.sheetUrl || 'not created yet'}`;
}

function formatMission(mission) {
  const shortfall = Number(mission.leads?.length || 0) < Number(mission.requested || 0)
    ? ` I found ${mission.leads?.length || 0} unique qualifying public LinkedIn profiles in this pass, below the requested ${mission.requested}.`
    : '';
  ...
  const deep = ` Selective public-page scraping used ${mission.deepSearches || 0} targeted search${mission.deepSearches === 1 ? '' : 'es'} and ${mission.deepFetches || 0} page fetch${mission.deepFetches === 1 ? '' : 'es'}.`;
  return `Lead mission complete, Sir. Created “${mission.spreadsheetTitle}” and added ${mission.added || 0} lead${mission.added === 1 ? '' : 's'} for “${mission.criteria}”.${contacts}${deep}${shortfall} ${mission.sheetUrl}`;
}
