...
async function executeApproved(compiled, missionId) {
  const mission = missionStore.update(missionId, { currentPhase: 'discovering', safetyState: 'approved' }) || missionStore.create(compiled);
  ...
  if (compiled.missionType === 'apollo_people_discovery') { const found = await people.discoverPeople(compiled); const rows = found.people; const projection = await projector.project(compiled, rows.map((p) => ({ name: p.organizationName || p.organization_name || 'Apollo people result', companyLink: p.organizationDomain ? 'https://' + p.organizationDomain : '', poc1: p })), { includePeople: true }); const final = missionStore.update(mission.missionId, { currentPhase: 'completed', completionReason: rows.length >= compiled.targetCount ? 'target_reached' : 'source_exhausted', peopleDiscovered: found.candidatesFound, peopleVerified: rows.length, companiesSelected: rows.length, rowsWritten: projection.rowsWritten, apolloCalls: found.apolloCalls, paidCalls: found.paidCalls, remainingTarget: Math.max(0, compiled.targetCount - rows.length), safetyState: 'complete' }); return response(true, `Source: Apollo\nPeople discovered: ${rows.length}\nRows written: ${projection.rowsWritten}\nApollo contact reveals: 0\n${formatPeopleLines(rows)}`, { mission: final, records: rows, projection }); }
...
async function handle(message, options = {}) {
  if (/\bapollo\s+(?:lead\s+)?(?:mission\s+)?(?:progress|status)\b/i.test(message)) { const latest = missionStore.latest(); return response(true, latest ? summary(latest) : 'No Apollo lead mission exists yet.', { mission: latest }); }
