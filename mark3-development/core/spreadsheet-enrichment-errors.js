...
function defaultCode(error, subsystem, type) {
  ...
  const s = text(subsystem).toUpperCase();
  const t = text(type).toUpperCase();

  const providerCodes = {
    APOLLO: {
      NETWORK: 'APOLLO_NETWORK_FETCH_FAILED',
      RATE_LIMIT: 'APOLLO_RATE_LIMIT',
      AUTH: 'APOLLO_AUTH_FAILED',
      PERMISSION: 'APOLLO_PERMISSION_DENIED',
      TIMEOUT: 'APOLLO_TIMEOUT',
      CONFIG: 'APOLLO_NOT_CONFIGURED',
      BAD_REQUEST: 'APOLLO_BAD_REQUEST',
      NOT_FOUND: 'APOLLO_RECORD_NOT_FOUND',
      API: 'APOLLO_API_ERROR',
    },
    GOOGLE_SHEETS: {
      NETWORK: 'GOOGLE_SHEETS_NETWORK_ERROR',
      RATE_LIMIT: 'GOOGLE_SHEETS_RATE_LIMIT',
      AUTH: 'GOOGLE_SHEETS_AUTH_FAILED',
      PERMISSION: 'GOOGLE_SHEETS_PERMISSION_DENIED',
      TIMEOUT: 'GOOGLE_SHEETS_TIMEOUT',
      CONFIG: 'GOOGLE_SHEETS_NOT_CONFIGURED',
      INVALID_RANGE: 'GOOGLE_SHEETS_INVALID_RANGE',
      NOT_FOUND: 'GOOGLE_SHEETS_RECORD_NOT_FOUND',
      API: 'GOOGLE_SHEETS_API_ERROR',
    },
    LINKEDIN: {
      NETWORK: 'LINKEDIN_NETWORK_ERROR',
      RATE_LIMIT: 'LINKEDIN_RATE_LIMIT',
      AUTH: 'LINKEDIN_AUTH_FAILED',
      PERMISSION: 'LINKEDIN_PERMISSION_DENIED',
      TIMEOUT: 'LINKEDIN_TIMEOUT',
      CONFIG: 'LINKEDIN_NOT_CONFIGURED',
      NOT_FOUND: 'LINKEDIN_RECORD_NOT_FOUND',
      API: 'LINKEDIN_API_ERROR',
    },
    GEMINI: {
      NETWORK: 'GEMINI_NETWORK_ERROR',
      RATE_LIMIT: 'GEMINI_RATE_LIMIT',
      AUTH: 'GEMINI_AUTH_FAILED',
      PERMISSION: 'GEMINI_PERMISSION_DENIED',
      TIMEOUT: 'GEMINI_TIMEOUT',
      BAD_REQUEST: 'GEMINI_BAD_REQUEST',
      API: 'GEMINI_API_ERROR',
    },
    GROQ: {
      NETWORK: 'GROQ_NETWORK_ERROR',
      RATE_LIMIT: 'GROQ_RATE_LIMIT',
      AUTH: 'GROQ_AUTH_FAILED',
      PERMISSION: 'GROQ_PERMISSION_DENIED',
      TIMEOUT: 'GROQ_TIMEOUT',
      BAD_REQUEST: 'GROQ_BAD_REQUEST',
      API: 'GROQ_API_ERROR',
    },
    NVIDIA: {
      NETWORK: 'NVIDIA_NETWORK_ERROR',
      RATE_LIMIT: 'NVIDIA_RATE_LIMIT',
      AUTH: 'NVIDIA_AUTH_FAILED',
      PERMISSION: 'NVIDIA_PERMISSION_DENIED',
      TIMEOUT: 'NVIDIA_TIMEOUT',
      BAD_REQUEST: 'NVIDIA_BAD_REQUEST',
      API: 'NVIDIA_API_ERROR',
    },
    BIG_PICKLE: {
      NETWORK: 'BIG_PICKLE_NETWORK_ERROR',
      TIMEOUT: 'BIG_PICKLE_TIMEOUT',
      CONFIG: 'BIG_PICKLE_NOT_CONFIGURED',
      API: 'BIG_PICKLE_API_ERROR',
    },
  };

  if (providerCodes[s]?.[t]) return providerCodes[s][t];
  if (t === 'NETWORK') return 'SPREADSHEET_NETWORK_ERROR';
  if (t === 'RECURSION') return 'UNIVERSAL_RECURSION_STACK_OVERFLOW';
  if (s === 'TARGETING') return 'WORKSHEET_TARGETING_FAILED';
  if (s === 'SCHEMA') return 'SPREADSHEET_SCHEMA_FAILED';
  if (s === 'CONTROL_PLANE') return 'ULTRON_ROUTING_FAILED';
