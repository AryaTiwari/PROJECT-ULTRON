...
const PROVIDERS = {
  ...
  // Browser, CLI, desktop-session and local-app bridges are intentionally excluded
  // from ordinary assistant chat. They can be enabled explicitly for experiments.
...
const PROVIDER_ALIASES = {
  ...
  ddgw: 'duckduckgo-web', felo: 'felo-web', tllm: 'theoldllm', unc: 'uncloseai',
  cfp: 'cloudflare-playground', cxa: 'codex-app-server', aug: 'auggie', zc: 'zcode',
  kr: 'kiro', if: 'qoder', qw: 'qwen', gh: 'github-copilot',
};
...
const BLOCKED_MODEL_PATTERNS = [
  ...
  /^nvidia\//i,
  /big[-_ ]?pickle/i,
  /(^|[\/_-])(dva|devin|agentic|bridge)([\/_-]|$)/i,
...
const MODEL_FAILURE_TTL = {
  MODEL_UNAVAILABLE: 6 * 60 * 60 * 1000,
  BAD_ROUTE: 60 * 60 * 1000,
  PAID_MODEL: 12 * 60 * 60 * 1000,
  UPSTREAM: 2 * 60 * 1000,
  RATE_LIMIT: 90 * 1000,
  ACCESS: 30 * 60 * 1000,
  UNKNOWN: 60 * 1000,
...
function isNonChatModel(model) {
  ...
  return NON_CHAT_PATTERNS.some((pattern) => pattern.test(value));
}
...
function allowUnknown() {
  return /^(1|true|yes|on)$/i.test(String(process.env.ULTRON_M3_ALLOW_UNKNOWN_PROVIDERS || ''));
}
...
function policyAllows(provider) {
  ...
  if (def.tier === 'experimental') return allowExperimental();
  return true;
}

function stateTemplate() {
  return { version: 2, providers: {}, models: {}, updatedAt: null };
}
...
function saveState(state) {
  ...
  writeJsonAtomic(config.providerHealthPath, state);
}
...
function modelAvailable(model, state = loadState()) {
  return !activeUntil(state.models?.[model]);
}

function providerAvailable(provider, state = loadState()) {
  return policyAllows(provider) && !activeUntil(state.providers?.[provider]);
}
...
async function credentialSnapshot() {
  ...
  for (const [provider, def] of Object.entries(PROVIDERS)) {
    const names = def.credentials || [];
    snapshot[provider] = names.some((name) => String(process.env[name] || stored[name] || '').trim());
  }
  return snapshot;
}
...
function taskModelScore(model, taskType = 'general') {
  ...
  if (/flash|mini|lite|small|fast/.test(value)) score += ['general', 'simple_qa', 'automation'].includes(String(taskType).toLowerCase()) ? 18 : 4;
  if (/chat|instruct/.test(value)) score += 8;
  if (/code|coder|coding/.test(value)) score += String(taskType).toLowerCase() === 'coding' ? 20 : 2;
  if (/reason|think|pro|sonnet/.test(value)) score += ['research', 'planning', 'coding'].includes(String(taskType).toLowerCase()) ? 14 : 3;
  return score;
}
...
async function buildCandidates(catalog, requestedModel = 'auto', taskType = 'general', options = {}) {
  ...
  const perProvider = Math.max(1, Number(process.env.ULTRON_M3_MODELS_PER_PROVIDER || 2));
  ...
  const explicit = requested && requested !== 'auto' && raw.includes(requested) && providerAvailable(providerFromModel(requested), state) && modelAvailable(requested, state)
    ? [requested]
    : [];
  ...
  for (const model of raw) {
    ...
    if (!providerAvailable(provider, state)) continue;
    if (!modelAvailable(model, state)) continue;
    if (!grouped.has(provider)) grouped.set(provider, []);
    grouped.get(provider).push(model);
  }

  const providerRows = [...grouped.keys()].map((provider) => {
    const def = PROVIDERS[provider] || { tier: 'unknown', priority: 500 };
    const health = state.providers?.[provider] || {};
    const recentSuccess = health.lastSuccessAt && Date.now() - Date.parse(health.lastSuccessAt) < config.providerHealthTtlMs;
    return {
      provider,
      priority: Number(def.priority || 500) - (creds[provider] ? 200 : 0) - (recentSuccess ? 300 : 0),
      recentSuccess,
      credentialDetected: Boolean(creds[provider]),
      healthyModel: health.healthyModel || null,
    };
  }).sort((a, b) => a.priority - b.priority || a.provider.localeCompare(b.provider));

  const ordered = [...explicit];
  for (const row of providerRows) {
    let models = grouped.get(row.provider) || [];
    models = models.sort((a, b) => {
      if (a === row.healthyModel && b !== row.healthyModel) return -1;
      if (b === row.healthyModel && a !== row.healthyModel) return 1;
      return taskModelScore(b, taskType) - taskModelScore(a, taskType) || a.localeCompare(b);
    });
    ordered.push(...models.slice(0, perProvider));
...
function recordFailure(model, kind, message) {
  ...
  state.models[model] = {
    ...
    lastFailureMessage: String(message || '').slice(0, 500),
    disabledUntil: new Date(now + modelTtl).toISOString(),
  };
  ...
  if (providerTtl) {
    state.providers[provider] = {
      ...
      lastFailureKind: kind,
      lastFailureMessage: String(message || '').slice(0, 500),
      disabledUntil: new Date(now + providerTtl).toISOString(),
    };
  }
  saveState(state);
}
...
async function snapshot(catalog = []) {
  const state = loadState();
  const creds = await credentialSnapshot();
  const catalogProviders = new Set((catalog || []).map(providerFromModel));
  const names = [...new Set([...Object.keys(PROVIDERS), ...catalogProviders])];
  ...
  return {
    ...
    unknownProvidersEnabled: allowUnknown(),
