...
function textFromResponse(data) {
  ...
  const content = choice?.message?.content ?? data?.response ?? data?.text ?? data?.content ?? '';
...
function contextBlock(userMessage, retrieved, commitments, decisions, projects, modelIntelligence) {
  return [
    'WORKING CONTEXT:', JSON.stringify({ recentMemories: retrieved, openCommitments: commitments.slice(0, 8), decisions: decisions.slice(0, 8), projects: projects.slice(0, 8) }),
    '', 'MODEL INTELLIGENCE:', JSON.stringify({ availableModelCount: modelIntelligence.live.count, models: modelIntelligence.live.models.slice(0, 60), observedPerformance: modelIntelligence.observed.slice(0, 40) }),
...
async function handle(message, options = {}) {
  ...
  try { intelligence = await models.intelligence(taskType); } catch (error) { emit('model_catalog_unavailable', { error: error.message }); }
  ...
  const data = await integrations.chat(messages, options.model || 'auto');
