const API='';
const CHAT_TIMEOUT_MS=45*60*1000;
...
function renderEvents(){els.activityList.innerHTML=state.events.slice(-10).reverse().map(e=>`<div class="event"><div class="event-state">${escapeHtml(e.type.replaceAll('_',' '))}</div><div class="event-label">${escapeHtml(e.label||e.reason||e.message||e.error||e.kind||(e.primary?`PRIMARY · ${e.primary}`:'')||'')}</div>${e.authUrl?`<a class="event-action" href="${escapeHtml(e.authUrl)}" target="_blank" rel="noreferrer">OPEN GOOGLE</a>`:''}${e.provider?`<div class="event-tool">${escapeHtml(e.provider)}</div>`:''}${e.tool?`<div class="event-tool">${escapeHtml(e.tool)}</div>`:''}${e.model?`<div class="event-tool">${escapeHtml(e.model)}</div>`:''}</div>`).join('');}
...
function connectEvents(){
  ...
  types.forEach(type=>es.addEventListener(type,e=>{try{
    ...
    else if(type==='model_candidate_started'){state.model=ev.model||state.model;els.model.textContent=state.model;els.hudModel.textContent=state.model;setState('ROUTING');}
