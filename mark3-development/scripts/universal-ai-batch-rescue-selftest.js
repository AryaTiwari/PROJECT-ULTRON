...
assert.match(operatorSource, /poc2Targets\.length && !aiFallbackEnabled/);
...
assert.match(controllerSource, /does not waste manual hydration attempts before AI/);
assert.match(controllerSource, /Apollo priority search, bounded broad\/brand retries/);
assert.match(controllerSource, /LinkedIn company lookup plus company-employees\/profile verification/);
assert.match(controllerSource, /Empty discovery results are never cached/);
