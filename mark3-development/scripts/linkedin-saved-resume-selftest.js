...
require.cache[config] = { id: config, filename: config, loaded: true, exports: { projectRoot: root } };
...
(async()=>{
  runner.start(async()=>{
    await runner.call('search_jobs',{keywords:'SAP'},async()=>({jobs:[{job_id:'123'}]}));
  ...
  runner.start(async prepared=>{
    ...
    assert.match(JSON.stringify(result),/123/);
    return {text:'done'};
  });
