...
async function main(){
 ...
 assert.equal(state.decide({completionGate:{complete:true,contactGaps:[gap,{rowNumber:3,groupOrdinal:3,field:'phone'}]},metrics:{pending:[{...gap,kind:'phone'}]}}),'TERMINAL_DATA_EXHAUSTED','one pending request cannot hide another unavailable contact');
