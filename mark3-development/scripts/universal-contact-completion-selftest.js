...
assert.match(operatorSource, /ULTRON_M3_APOLLO_PHONE_DIRECT_BATCH_LIMIT/);
assert.match(operatorSource, /Prefer the callback worker's batch store/);
assert.match(operatorSource, /apolloPhonePollRateLimited\(error\)/);
assert.match(operatorSource, /ULTRON_M3_APOLLO_PHONE_WATCHER_ATTEMPTS \|\| 120/);
const serverSource=fs.readFileSync(path.join(__dirname,'..','server.js'),'utf8');
assert.match(serverSource, /error\?\.code !== 'EADDRINUSE'/);
assert.match(serverSource, /ULTRON is already running at http:/);
