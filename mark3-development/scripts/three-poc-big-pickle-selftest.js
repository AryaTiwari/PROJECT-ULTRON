...
const fallbackIndex = targetedSource.indexOf('exactRowLastResort(exact.request, primary, runOptions)');
...
assert.ok(fallbackIndex > primaryIndex, 'exact-row deterministic last resort must remain behind deterministic primary');
assert.match(targetedSource, /targetOrdinals:\s*undefined/);
