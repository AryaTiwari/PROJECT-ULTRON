...
const indianCompanyLocationRequest = operator.parseRequest('Find companies in India on LinkedIn that are hiring SAP under 1000 employees');
assert.equal(indianCompanyLocationRequest.location, 'India');
assert.equal(indianCompanyLocationRequest.locationScope, 'company');
const indiaJobLocationRequest = operator.parseRequest('Find SAP jobs on LinkedIn in India under 1000 employees');
assert.equal(indiaJobLocationRequest.locationScope, 'job');
...
const meghalayaPlan = operator.jobSearchPlan({ ...filtered, location: 'Meghalaya' });
assert.equal(meghalayaPlan[0].location, 'Meghalaya');
assert.ok(meghalayaPlan.some((item) => item.location === 'Shillong'));
const puducherryPlan = operator.jobSearchPlan({ ...filtered, location: 'Puducherry' });
assert.equal(puducherryPlan[0].location, 'Puducherry');
