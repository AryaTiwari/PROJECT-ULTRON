...
(async () => {
  ...
  console.log('3-POC internal inference self-test passed. Universal deterministic spreadsheet, typed-error taxonomy, Apollo transport retry, verified contact completion, Google core-API and values-range hardening, routing, approval, contact-display, ordinal-contact, orphan-contact, exact Arya 2 layout and bounded Big Pickle fallback regressions also ran; personal direct-model calls stay blocked.');
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});