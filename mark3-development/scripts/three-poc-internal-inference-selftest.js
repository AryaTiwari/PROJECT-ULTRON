...
require('./three-poc-candidate-discovery-selftest');
...
(async () => {
  ...
  console.log('3-POC internal inference self-test passed. Universal deterministic spreadsheet, high-recall/current-employer-safe discovery, typed-error taxonomy, enrichment diagnostics, Apollo transport retry, native-phone/contact-quality policy, verified contact completion, schema continuity recovery, bounded direct-env AI batch rescue, Google core-API and values-range hardening, routing, approval, contact-display, ordinal-contact, orphan-contact, exact Arya 2 layout and bounded Big Pickle fallback regressions also ran; direct env-backed models are permitted only inside authorized spreadsheet inference, while OmniRoute remains blocked there.');
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});