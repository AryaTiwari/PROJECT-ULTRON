...
// classifier outside the normal HTTP ownership path.
assert.throws(
  ...
  (error) => error && error.code === 'DOMAIN_ROUTE_INVARIANT_VIOLATION'
);
...
(async () => {
  ...
  console.log('3-POC internal inference self-test passed. Internal JSON is not reclassified as a LinkedIn user command; scoped OmniRoute reasoning remains allowed while direct personal-model calls remain blocked.');
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
