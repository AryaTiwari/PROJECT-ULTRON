...
function formatGitHubReadResult(tool, result) {
  if (!result?.ok) return { ok: false, response: `I couldn't read ${tool.input.path} from GitHub. ${result?.error || ''}`.trim(), tool_result: result };
  const payload = result.result || result;
  if (tool.input.path.toLowerCase().endsWith('core/personality/default.json')) {
    try {
      const cfg = JSON.parse(payload.content || '');
      const instructions = Array.isArray(cfg.ULTRON_INSTRUCTIONS) ? cfg.ULTRON_INSTRUCTIONS : Array.isArray(cfg.behavioralInstructions) ? cfg.behavioralInstructions : [];
      const instructionText = instructions.length ? ` Behavioral directives: ${instructions.length} are configured.` : '';
      return { ok: true, response: `I read ${payload.path} from ${payload.branch || 'GitHub'}. I am ${cfg.ULTRON_NAME || cfg.name || 'ULTRON'}, operating as ${cfg.ULTRON_ROLE || cfg.role || 'Arya\'s personal AI assistant and strategic companion'}. My personality is ${cfg.ULTRON_PERSONALITY || cfg.personality || 'calm, formidable, strategic, creative, philosophical, direct, subtly playful, and deeply practical'}.${instructionText} I challenge weak ideas and avoidance, stay composed, use dry situational humor, push when you're procrastinating, and prioritize truth, practical action, creativity, and continuity.` , tool_result: result };
    } catch {}
  }
  return { ok: true, response: `I read ${payload.path} from ${payload.branch || 'GitHub'}.\n\n${String(payload.content || '').trim()}`.trim(), tool_result: result };
}

class Mark2Runtime extends UltronCore{
  async handleMessage(message,options={}){
    const intent=parseToolIntent(message);
    if(intent){const risky=['run_powershell','write_file'].includes(intent.name);const guardian=require('./guardian').assess({message,action:risky?{destructive:true,requiresConfirmation:true}:null});const critic=require('./critic').analyze({message,plannedAction:risky?{destructive:true,externalSideEffect:true}:null},guardian);if(guardian.decision==='block'){updateMood(message,{response:'blocked'});return{ok:true,blocked:true,response:guardian.reasons.join(' '),guardian,critic};}if(guardian.decision==='warn'&&options.confirmed!==true){updateMood(message,{response:'warning'});return{ok:true,requires_confirmation:true,response:`Guardian warning: ${guardian.reasons.join(' ')}`,guardian,critic,tool:intent};}const result=await execute(intent.name,intent.input,{confirmed:options.confirmed===true,source:options.source||'core'});const mood=updateMood(message,result);if(intent.name==='github_read_file')return{...formatGitHubReadResult(intent,result),guardian,critic,mood};return{ok:result.ok,response:result.ok?(intent.name==='speak_text'?'Voice synthesis completed.':`${intent.name} completed.`):result.error,tool_result:result,guardian,critic,mood};}
  ...
  async handleMessageStream(message, options={}, onEvent=()=>{}) {
    const intent=parseToolIntent(message);
    if(intent){
      if(intent.name==='github_read_file') onEvent({type:'meta',state:'researching',label:`Reading ${intent.input.path} from GitHub.`,tool:intent.name});
      else if(intent.name==='speak_text') onEvent({type:'meta',state:'speaking',label:'Preparing voice output.',tool:intent.name});
      else onEvent({type:'meta',state:'executing',label:`Executing ${intent.name}.`,tool:intent.name});
      const result=await this.handleMessage(message,options);
      onEvent({type:'tool',state:'executing',label:`${intent.name} complete.`,tool:intent.name,toolResults:[{tool:intent.name,ok:result?.ok!==false}]});
      onEvent({type:'final',state:'responding',label:'Final response ready.',result});
      return;
    }
