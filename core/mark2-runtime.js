...
const { execute, openAITools } = require('./executor');
...
function mergeToolCalls(toolCalls = []) {
  ...
  for (const call of Array.isArray(toolCalls) ? toolCalls : []) {
    ...
    const existing = merged.get(index) || { index, id: call?.id || null, type: call?.type || 'function', function: { name: '', arguments: '' } };
...
async function executeToolCalls(toolCalls, options = {}) {
  ...
  for (const toolCall of mergeToolCalls(toolCalls)) {
    let input = {};
    try { input = typeof toolCall.function.arguments === 'string' ? JSON.parse(toolCall.function.arguments || '{}') : (toolCall.function.arguments || {}); }
    catch { results.push({ toolCall, result: { ok: false, error: `Invalid tool arguments for ${toolCall.function.name}.` } }); continue; }
    const result = await execute(toolCall.function.name, input, {
...
class Mark2Runtime extends UltronCore{
  async handleMessage(message,options={}){
    ...
    const base=this.buildMessages(workingMessage,await this.getRelevantMemories(workingMessage,8),require('./memory/local-store').getRecentMessages());
    let messages=base; let lastResult=null; const tools=openAITools();
  ...
  async handleMessageStream(message, options={}, onEvent=()=>{}) {
    const intent=parseToolIntent(message); if(intent){ const result=await this.handleMessage(message,options); onEvent({type:'final',result}); return; }
    const userMessage=String(message||'').trim(); if(!userMessage) { onEvent({type:'error',error:'Message is required.'}); return; }
    const task=require('./task-classifier').classify(userMessage); const selectedModel=require('./model-policy').selectModel(userMessage,options.model); const guardian=require('./guardian').assess({message:userMessage,action:options.action||null}); const critic=require('./critic').analyze({message:userMessage,plannedAction:options.action||null},guardian); if(guardian.decision==='block'||(guardian.decision==='warn'&&options.confirmed!==true)||(critic.status==='blocked')){ const result={ok:true,response:guardian.decision==='block'?`I can't execute that request. ${guardian.reasons.join(' ')}`:guardian.decision==='warn'?`Guardian warning: ${guardian.reasons.join(' ')}`:'The request needs a safer approach before execution.', requires_confirmation:guardian.decision!=='block', guardian,critic,task,model:selectedModel}; onEvent({type:'final',result}); return; }
    ...
    for(let round=0; round<=maxToolRounds; round+=1){
      ...
      try {
        streamResult=await router.streamChat({messages,model:selectedModel,taskType:task.taskType,tools,onDelta:(text,meta)=>{full+=text;onEvent({type:'delta',text,...meta});}});
      } catch(error){ await require('./telemetry').recordModelResult({model:selectedModel,taskType:task.taskType,success:false,latencyMs:Date.now()-started,errorType:error?.name||'model_error',metadata:{message:String(error?.message||error).slice(0,500),streamedChars:full.length}}); onEvent({type:'error',error:error.message,status:error.status||null,partial:Boolean(full)}); return; }
      const toolCalls=mergeToolCalls(streamResult.toolCalls);
      if(!toolCalls.length){ finalResult={ok:true,response:streamResult.content,model:streamResult.model,task,guardian,critic,memory:memoryResults,relevant_memories:relevantMemories,tools:require('./executor').listTools()}; break; }
      const toolResults=await executeToolCalls(toolCalls,options);
      onEvent({type:'tool',toolCalls,toolResults});
