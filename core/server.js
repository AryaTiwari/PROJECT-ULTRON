...
const { getVoiceModel } = require('./voice/fish-model');
...
function send(res, status, payload, contentType = 'application/json; charset=utf-8') {
  const body = typeof payload === 'string' || payload instanceof Buffer ? payload : JSON.stringify(payload);
...
function readBody(req) {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', c => { raw += c; if (raw.length > 100000) reject(new Error('Request body too large.')); });
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && req.url === '/api/tts/play') {
      ...
      return send(res, 200, { ...audio, playback: await playLocalAudio(audio.path) });
    }
