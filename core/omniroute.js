...
function isOpenCodeModel(model) {
  ...
  return value.startsWith('opencode/') || value.startsWith('oc/') || value.includes('big-pickle') || value.includes('big_pickle') || value.includes('big pickle');
}
...
async function hasModel(model) { const target = normalizeModelId(model); if (!target || (mark3OpenCodeDisabled() && isOpenCodeModel(target))) return false; try { return (await listModels()).includes(target); } catch { return false; } }
...
async function resolveModel(requestedModel = 'auto', taskType = 'general') {
  ...
  if (requested && requested !== 'auto') {
    if (mark3OpenCodeDisabled() && isOpenCodeModel(requested)) throw new Error(`OpenCode provider is disabled for Mark 3: ${requested}`);
    return requested;
  }
  let models = [];
  ...
  // Mark 3 disabled OpenCode: never hand an `auto` alias back to OmniRoute,
  // because that would let the gateway choose OpenCode internally.
  const concrete = models.filter((id) => !/^auto(?:\/|$)/i.test(id) && !isOpenCodeModel(id));
  const preferredPrefixes = ['nvidia/', 'nvidia:', 'nvidia-', 'gemini/', 'openai/', 'anthropic/', 'deepseek/', 'groq/'];
  for (const prefix of preferredPrefixes) {
    const hit = concrete.find((id) => id.toLowerCase().startsWith(prefix));
    if (hit) return hit;
  }
  if (concrete.length) return concrete[0];
  throw new Error('Mark 3 has OpenCode disabled and OmniRoute published no other concrete model.');
}
...
async function streamChat({ messages, model = 'auto', taskType = 'general', tools = null, onDelta, firstTokenTimeoutMs = null } = {}) { if (!Array.isArray(messages) || !messages.length) throw new Error('OmniRoute streaming request requires messages.'); if (typeof onDelta !== 'function') throw new Error('OmniRoute streaming request requires an onDelta callback.'); const resolvedModel = await resolveModel(model, taskType); const configuredFirstTokenTimeout = Number(firstTokenTimeoutMs || process.env.ULTRON_STREAM_FIRST_TOKEN_TIMEOUT_MS || 15000); const qualitySensitive = ['coding', 'research', 'planning'].includes(String(taskType || '').toLowerCase()); const candidates = mark3OpenCodeDisabled() ? [resolvedModel] : [...new Set([resolvedModel, ...(qualitySensitive ? [] : ['auto/best-fast']), ...(resolvedModel !== 'auto' ? ['auto'] : [])])]; let lastError = null;
  for (const candidate of candidates) {
    if (mark3OpenCodeDisabled() && isOpenCodeModel(candidate)) continue;
    for (let attempt = 0; attempt < 2; attempt += 1) {
      let controller = null; let firstTokenTimer = null; let gotDelta = false;
      try {
        const key = await apiKey(); controller = new AbortController(); firstTokenTimer = setTimeout(() => { if (!gotDelta) controller.abort(); }, configuredFirstTokenTimeout);
        const url = `${baseUrl()}/chat/completions`; const response = await ultronFetch(url, { method: 'POST', headers: headers(key, true), body: JSON.stringify({ model: candidate, messages, stream: true, ...(Array.isArray(tools) && tools.length ? { tools } : {}) }), signal: controller.signal });
        ...
        const consume = (block) => { const event = parseSseBlock(block); if (!event || event === '[DONE]') return; let data; try { data = JSON.parse(event); } catch { return; } const choice = data?.choices?.[0] || {}; const delta = choice?.delta || {}; const text = textFromContent(delta.content || delta.text || delta.reasoning_content || choice.text || data?.output_text || data?.text || ''); if (text) { if (mark3OpenCodeDisabled() && isOpenCodeModel(data?.model)) throw new Error(`OmniRoute streaming selected disabled OpenCode model: ${data?.model}`); gotDelta = true; fullText += text; onDelta(text, { model: data?.model || candidate, finishReason: choice?.finish_reason || null, firstTokenMs: fullText.length === text.length ? Date.now() : undefined }); } if (Array.isArray(delta.tool_calls)) toolCalls.push(...delta.tool_calls); if (choice?.finish_reason) finishReason = choice.finish_reason; };
        for await (const chunk of response.body) { buffer += decoder.decode(chunk, { stream: true }); let boundary; while ((boundary = findSseBoundary(buffer)) >= 0) { const separatorLength = sseBoundaryLength(buffer, boundary); const block = buffer.slice(0, boundary); buffer = buffer.slice(boundary + separatorLength); consume(block); } }
        buffer += decoder.decode(); if (buffer.trim()) consume(buffer); if (!gotDelta && !toolCalls.length) { const error = new Error('OmniRoute streaming returned no usable content.'); error.status = 502; throw error; }
        return { content: fullText, toolCalls, finishReason, model: resolvedModel, provider: 'omniroute', requestedModel: model, taskType };
      } catch (error) { if (firstTokenTimer) clearTimeout(firstTokenTimer); lastError = error; const abortedForFirstToken = error?.name === 'AbortError' && !gotDelta; if (abortedForFirstToken) { if (attempt === 0) { await new Promise(resolve => setTimeout(resolve, 300)); continue; } if (qualitySensitive) throw new Error(`OmniRoute first token exceeded ${configuredFirstTokenTimeout}ms for ${candidate}.`); break; } if (!transientStatus(error?.status)) throw error; if (attempt === 0) await new Promise(resolve => setTimeout(resolve, 350)); }
    }
  }
