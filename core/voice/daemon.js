...
function startVoiceDaemon() {
  ...
  state.running = true; state.pid = child.pid;
      ...
      pipeline.processTranscript(event.text, message => core.handleMessage(message)).then(async result => {
        if (result?.audio?.path) {
          ...
          if (process.env.ULTRON_VOICE_AUTOPLAY !== 'false') {
            try { const audio = await synthesize(result.result.response); spawn('powershell.exe', ['-NoProfile', '-Command', `Start-Process -FilePath '${String(audio.path).replace(/'/g, "''")}'`], { windowsHide: true, detached: true, stdio: 'ignore' }).unref(); }
...
function stopVoiceDaemon() {
  ...
  child.kill(); child = null; state.running = false; state.pid = null; return { ok: true, stopped: true, ...status() };
}
