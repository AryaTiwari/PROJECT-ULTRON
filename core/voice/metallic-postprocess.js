...
async function processMetallic(inputPath, outputPath = inputPath) {
  ...
  const filter = `asplit=2[a][b];[a]adelay=${delayMs}|${delayMs},aecho=1:0.8:${delayMs}:${feedback}[wet];[b][wet]amix=inputs=2:weights=${dry} ${wet}:normalize=0,volume=4.0,alimiter=limit=0.97`;

  try {
    await execFileAsync('ffmpeg', ['-y', '-i', inputPath, '-filter_complex', filter, '-codec:a', 'libmp3lame', '-q:a', '2', tempPath], { windowsHide: true, timeout: 120000, maxBuffer: 1024 * 1024 });
    fs.renameSync(tempPath, outputPath);
    return { applied: true, path: outputPath, mix, loudnessBoost: 4.0, limiter: 0.97 };
  } finally {
