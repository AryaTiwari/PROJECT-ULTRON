...
async function ensureReferenceWav() {
  ...
  if (!fs.existsSync(source)) throw new Error(`Voice reference not found: ${source}`);
...
async function synthesize(text, options = {}) {
  ...
  if (!apiKey) throw new Error('NVIDIA API key is not configured. Add NVIDIA_API_KEY through ULTRON credentials or .env.');
  const reference = await ensureReferenceWav();
  const form = new FormData();
  form.append('language', options.language || 'en-US');
  form.append('text', input.slice(0, 2000));
  ...
  const response = await fetch(NVIDIA_API_URL, { method: 'POST', headers: { Authorization: `Bearer ${apiKey}` }, body: form });
  ...
  return { ok: true, provider: 'nvidia-magpie-zeroshot', model: 'nvidia/magpie-tts-zeroshot', referencePath: config.referencePath, path: processed.path || outputPath, bytes: fs.statSync(processed.path || outputPath).size, metallicApplied: processed.applied, metallicMix: config.metallicMix };
}

module.exports = { synthesize, ensureReferenceWav, NVIDIA_API_URL, NVIDIA_STREAM_URL };
