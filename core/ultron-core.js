...
const memoryPolicy = require('./memory/update-policy');
...
class UltronCore {
  ...
  status() {
    return {
      ...
      ready: true,
      layers: { guardian: 'ready', critic: 'ready', executor: 'ready', memory: supabase.available() ? 'supabase+local' : 'local-fallback', model_router: 'ready' },
  ...
  async getMemories() {
    if (supabase.available()) {
      try { const memories = await supabase.listMemories(500); if (Array.isArray(memories)) return memories; } catch { /* local fallback */ }
    }
  ...
  async rememberCandidate(candidate) {
    ...
    const conflict = memories.find(memory => memoryPolicy.shouldSupersede(candidate, memory));
    if (conflict && (candidate.explicitCorrection || Number(candidate.confidence ?? 0) >= Number(conflict.confidence ?? 0))) {
      ...
      local.saveMemory(replacement);
      local.supersedeMemory(conflict.id, replacement.id);
      if (supabase.available()) {
        try { await supabase.insertMemory(replacement); await supabase.updateMemory(conflict.id, { active: false, superseded_by: replacement.id, updated_at: new Date().toISOString() }); } catch { /* local fallback retained */ }
      }
      return { decision: 'UPDATE', reason: 'conflicting_memory_superseded', matched: conflict, saved: true, memory: replacement };
    }
    ...
    const memory = {
      id: id(), memory_type: candidate.type || 'fact', content: candidate.content,
      normalized_content: decision.normalized, content_hash: decision.content_hash,
      importance: candidate.importance ?? 0.5, confidence: candidate.confidence ?? 0.8,
      source: candidate.source || 'conversation', active: true, created_at: new Date().toISOString(), updated_at: new Date().toISOString(), metadata: {},
    };
    local.saveMemory(memory);
    if (supabase.available()) { try { await supabase.insertMemory(memory); } catch { /* local fallback retained */ } }
  ...
  buildMessages(userMessage, memories, recent) {
    ...
    const context = ['RELEVANT LONG-TERM MEMORY:', memoryText, '', 'RECENT CONVERSATION:', conversationText].join('\n');
  ...
  async handleMessage(message, options = {}) {
    ...
    if (supabase.available()) { try { await supabase.insertConversationMessage({ role: 'user', content: userMessage, metadata: { task_type: task.taskType }, created_at: timestamp }); } catch { /* local fallback */ } }
    ...
    if (guardian.decision === 'warn' && options.confirmed !== true) return { ok: true, requires_confirmation: true, response: `Guardian warning: ${guardian.reasons.join(' ')}`, guardian, critic, task, model: selectedModel };
    ...
    if (supabase.available()) { try { await supabase.insertConversationMessage({ role: 'assistant', content: result.content, model: result.model, metadata: { task_type: task.taskType }, created_at: createdAt }); } catch { /* local fallback */ } }
