...
class UltronCore {
  ...
  async rememberCandidate(candidate) {
    const memories = await this.getMemories(); const decision = await memoryJudge.judge(candidate, memories); if (decision.decision !== 'SAVE') return decision;
    const memory = { id: id(), memory_type: candidate.type || 'fact', content: candidate.content, normalized_content: decision.normalized, content_hash: decision.content_hash, importance: candidate.importance ?? 0.5, confidence: candidate.confidence ?? 0.8, source: candidate.source || 'conversation', active: true, created_at: new Date().toISOString(), updated_at: new Date().toISOString(), metadata: {} };
    local.saveMemory(memory); if (supabase.available()) { try { await supabase.insertMemory(memory); } catch {} } return { ...decision, saved: true, memory };
  }
  ...
  async handleMessage(message, options = {}) {
    ...
    if (guardian.decision === 'block') { const response = `I can't execute that request. ${guardian.reasons.join(' ')}`; local.appendConversation({ id: id(), role: 'assistant', content: response, created_at: new Date().toISOString() }); return { ok: true, response, blocked: true, guardian, critic, task }; }
