...
async function checkGithub() {
  ...
  return result('github', probe.ok ? (token ? 'CONNECTED' : 'REACHABLE_UNAUTHENTICATED') : 'OFFLINE', {
    configured: Boolean(token), authenticated: Boolean(token && probe.ok), latencyMs: probe.latencyMs, httpStatus: probe.statusCode, error: probe.error,
  });
}
...
async function checkInstagram() {
  ...
  return result('instagram', probe.ok ? 'CONNECTED' : 'AUTH_OR_NETWORK_ERROR', {
    configured: true, authenticated: probe.ok, userIdConfigured: Boolean(userId), latencyMs: probe.latencyMs, httpStatus: probe.statusCode, error: probe.error,
  });
}
...
async function checkOmniRoute() {
  ...
  return result('omniroute', probe.ok || probe.statusCode === 401 ? (probe.statusCode === 401 ? 'ONLINE_AUTH_REQUIRED' : 'ONLINE') : 'OFFLINE', {
    configured: true, endpoint: base, latencyMs: probe.latencyMs, httpStatus: probe.statusCode, error: probe.error,
  });
}

async function checkMemory() {
  const dataDir = path.resolve(process.env.ULTRON_DATA_DIR || '.ultron');
  const memoryDir = path.join(dataDir, 'memory');
  const supabaseConfigured = Boolean(process.env.SUPABASE_URL && (process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY));
  try { fs.mkdirSync(dataDir, { recursive: true }); } catch {}
  return result('memory', supabaseConfigured ? 'LOCAL_READY_SUPABASE_CONFIGURED' : 'LOCAL_READY', {
    localPath: dataDir, memoryDirectory: memoryDir, supabaseConfigured, writable: (() => { try { const p = path.join(dataDir, '.write-test'); fs.writeFileSync(p, 'ok'); fs.rmSync(p, { force: true }); return true; } catch { return false; } })(),
  });
}
...
async function checkInternetSpeed() {
  ...
  try {
    ...
    let bytes = 0; for await (const chunk of response.body) bytes += Buffer.byteLength(chunk);
    const elapsedSeconds = Math.max((Date.now() - started) / 1000, 0.001);
    return result('internet_speed', 'CONNECTED', { latencyMs: Date.now() - started, measuredMbps: Number(((bytes * 8) / elapsedSeconds / 1000000).toFixed(2)), bytes });
  } catch (error) { return result('internet_speed', 'OFFLINE_OR_BLOCKED', { error: error?.message || String(error) }); }
  finally { clearTimeout(timer); }
}

async function checkMood() {
  try {
    const moodPath = path.resolve(process.env.ULTRON_MOOD_FILE || '.ultron/mood.json');
  ...
  } catch { return result('mood', 'CALM', { source: 'fallback' }); }
}
...
async function collectSystemStatus() {
  ...
  return { ok: true, service: 'ultron-status', version: 'mark2', timestamp: new Date().toISOString(), status: { mood, github, instagram, administrator, omniroute, internetSpeed, memory }, voice: { provider: voiceConfig.provider, model: voiceConfig.model, referenceId: voiceConfig.referenceId, metallicMix: voiceConfig.metallicMix } };
}
