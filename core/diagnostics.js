...
async function diagnose() {
  ...
  let saved = {};
  try { saved = await credentialStore.load(); } catch {}
  const omniKey = String(saved.OMNIROUTE_API_KEY || config.router.apiKey || '').trim();
  ...
  checks.push({ name: 'omniroute_key', ok: Boolean(omniKey), source: saved.OMNIROUTE_API_KEY ? 'local-dpapi' : config.router.apiKey ? 'environment' : 'none' });
