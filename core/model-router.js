...
async function streamChat({messages,model,tools=null,taskType='general',onDelta}={}){
  ...
  if(isOmniRouteModel(requestedModel)||mode==='omniroute'||mode==='auto'){
    try{return await omniRoute.streamChat({messages,model:requestedModel,taskType,tools,onDelta});}
    catch(error){if(mode!=='auto')throw error; const fallback=await chat({messages,model:requestedModel,tools,taskType});onDelta(fallback.content,{model:fallback.model,finishReason:'stop',fallback:true});return fallback;}
  }
